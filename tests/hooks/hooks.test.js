...
async function runTests() {
  ...
  if (test('script references use DROID_PLUGIN_ROOT variable', () => {
    const hooksPath = path.join(__dirname, '..', '..', 'hooks', 'hooks.json');
    const hooks = JSON.parse(fs.readFileSync(hooksPath, 'utf8'));
      ...
      for (const entry of hookArray) {
        for (const hook of entry.hooks) {
          if (hook.type === 'command' && hook.command.includes('scripts/hooks/')) {
            // Check for the literal string "${DROID_PLUGIN_ROOT}" in the command
            const hasPluginRoot = hook.command.includes('${DROID_PLUGIN_ROOT}');
            assert.ok(
              hasPluginRoot,
              `Script paths should use DROID_PLUGIN_ROOT: ${hook.command.substring(0, 80)}...`
            );
          }
        }
      }
