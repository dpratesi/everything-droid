...
Manage Claude Code session history - list, load, alias, and edit sessions stored in `~/.factory/sessions/`.
...
```bash
node -e "
const sm = require((process.env.DROID_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.factory'))+'/scripts/lib/session-manager');
const aa = require((process.env.DROID_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.factory'))+'/scripts/lib/session-aliases');
...
console.log('Path: ~/.factory/sessions/' + session.filename);
console.log('');
...
- Sessions are stored as markdown files in `~/.factory/sessions/`
